...
export default function Home() {
  ...
  const handleCalculateCpm = () => {
    ...
    if (startTasks.length > 1) {
      warnings.push(
        `⚠ Wiele czynności startowych (${startTasks.map((t) => t.name).join(", ")}). W klasycznej CPM zaleca się jedno zdarzenie początkowe.`,
      );
    }
    if (endTasks.length > 1) {
      warnings.push(
        `⚠ Wiele czynności końcowych (${endTasks.map((t) => t.name).join(", ")}). W klasycznej CPM zaleca się jedno zdarzenie końcowe.`,
      );
    }
        ...
        <View style={styles.precedentsContainer}>
          {tasks
            ...
            .map((t) => (
              ...
              >
                <Text
                  style={
                    selectedPrecedents.includes(t.name)
          ...
          {tasks.length === 0 && (
            <Text style={{ color: "#999" }}>
              Najpierw dodaj jakieś czynności
            </Text>
          )}
        </View>
