...
export function computeLayout(
  ...
  const maxNodesAtLevel = Math.max(
    ...[...levelGroups.values()].map((g) => g.length),
  );

  const totalWidth =
    (maxLevel + 1) * (NODE_WIDTH + LEVEL_GAP) - LEVEL_GAP + 2 * PADDING;
  const totalHeight =
    maxNodesAtLevel * (NODE_HEIGHT + NODE_GAP) - NODE_GAP + 2 * PADDING;
  ...
  for (const [level, names] of levelGroups.entries()) {
    ...
    const groupHeight =
      names.length * NODE_HEIGHT + (names.length - 1) * NODE_GAP;
  ...
  for (const task of tasks) {
    ...
    for (const predName of task.precedents) {
      ...
      edges.push({
        ...
        critical:
          (fromResult?.critical ?? false) && (toResult?.critical ?? false),
      });
    }
  }
